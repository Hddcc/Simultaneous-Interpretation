...
桌面端 AI 同声传译助手。打开声桥后，可以把浏览器视频、桌面会议、通话软件、麦克风或本地音视频文件里的声音转成中英双语字幕，并通过悬浮字幕窗口覆盖在其他应用上方。近期字幕会随着 ASR final、翻译修正和重连恢复自动更新，帮助你跟上外语内容节奏。
...
| 字幕自动修订 | recent 字幕会根据 ASR partial/final、翻译修正和 provider 恢复状态原地更新 |
| 悬浮字幕窗口 | 字幕窗口可覆盖在浏览器、会议或通话窗口上方，适合边看边读 |
| 可选译文播报 | 稳定译文可进入本地语音播报队列，支持暂停和停止 |
| 服务状态可见 | 工作台显示 provider、API Key、连接状态、队列深度、延迟和可恢复错误 |
...
2. 在声桥中选择“系统音频”。
3. 点击刷新来源，选择浏览器窗口、播放器窗口或整个屏幕。
4. 点击“开始”，观察音量、Payload、ASR 延迟、翻译延迟和字幕历史。
5. 打开“悬浮字幕”，把字幕窗口放到视频上方。
...
文件模拟适合调试 API、演示固定素材，以及对比字幕修订效果。它不会替代桌面系统音频场景，但能稳定复现完整同传链路。
...
| 悬浮字幕 | 可用 | 可覆盖其他应用，部分窗口管理器可能影响置顶行为 |
...
最近几条字幕会随着 ASR final 和翻译修正更新，这是为了降低实时识别误差。较早字幕会逐步稳定，避免历史记录持续跳动。

### 真实延迟大概由什么决定？

主要由音频块时长、网络连接、ASR 响应、翻译模型、队列压力和字幕修订策略共同决定。工作台会展示 ASR 延迟、翻译延迟、总延迟和队列深度，便于判断瓶颈。
...
真实桌面体验建议按 `docs/verification/desktop-scenarios.md` 检查：浏览器视频、会议软件、通话软件、麦克风 fallback、悬浮字幕和可选译文播报。
