...
DeepSeek-compatible provider 用于翻译已经识别出的文本。完整实时同传仍需要 ASR provider 将音频转换为文字。
...
`.env` 只用于本地运行，真实 API Key 不应提交到仓库。渲染进程只接收密钥是否存在、缺失项和 provider 状态，实际密钥读取与 provider 调用位于 Electron 主进程。
